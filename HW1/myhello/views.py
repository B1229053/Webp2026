...
def add_course(request):
    v_dept = request.GET.get('dept', '')
    v_title = request.GET.get('title', '')
    v_teacher = request.GET.get('instructor', '')

    if v_title:
        Course.objects.create(
            Department=v_dept,
            CourseTitle=v_title,
            Instructor=v_teacher
        )
        return Response({"res": f"課程 {v_title} 已加入！"})
    return Response({"res": "請輸入課程名稱"}, status=400)
